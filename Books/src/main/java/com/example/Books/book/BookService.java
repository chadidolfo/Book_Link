...
import com.example.Books.exeption.OperationNotPermittedException;
import com.example.Books.file.FileStorageService;
...
public class BookService {
    ...
    public Integer updateShareableStatus(Integer bookId, Authentication connectedUser) {
            ...
            Book book = bookRepository.findById(bookId)
                    .orElseThrow(() -> new EntityNotFoundException("No book found with ID:: " + bookId));
            User user = ((User) connectedUser.getPrincipal());
            if (!Objects.equals(book.getOwner().getId(), user.getId())) { //compare book.get.. & user.get..

                throw new OperationNotPermittedException("You cannot update others books shareable status");
            }
            book.setShareable(!book.isShareable());//the book is not shareable
    ...
    public Integer borrowBook(Integer bookId, Authentication connectedUser) {
            ...
            // fetch the book                           make sure we have the book in DB
            Book book = bookRepository.findById(bookId)
                    .orElseThrow(() -> new EntityNotFoundException("No book found with ID:: " + bookId));

            //check if the book is not archived and is shareable
           ...
           //check that the user isn't the same as the owner
            User user = ((User) connectedUser.getPrincipal());
            if (Objects.equals(book.getOwner().getId(), user.getId())) {
            ...
            if (isAlreadyBorrowedByUser) {
                throw new OperationNotPermittedException("You already borrowed this book and it's still not returned or the return is not approved by the owner");
            }
